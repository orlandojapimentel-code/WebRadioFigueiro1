...
const NewsTicker: React.FC = () => {
    ...
    try {
      ...
      if (newsBlocks) {
        const titles = newsBlocks.map(block => {
          const match = block.match(/TITULO:\s*(.*)/i);
  ...
  if (loading && newsText.length === 0) return null;

  return (
    <div className="fixed top-20 left-0 right-0 z-40 bg-slate-900/90 dark:bg-black/80 backdrop-blur-2xl border-b border-white/5 h-11 flex items-center overflow-hidden shadow-2xl">
      ...
      <div className="flex-grow relative overflow-hidden h-full flex items-center">
        <div className="animate-ticker flex whitespace-nowrap items-center space-x-20">
          {(newsText.length > 0 ? newsText.concat(newsText) : ["Web Rádio Figueiró: A sintonizar as notícias de Amarante e do Mundo..."]).map((text, i) => (
            <div key={i} className="flex items-center space-x-6">
              <span className="text-white dark:text-gray-100 text-[11px] font-bold tracking-tight uppercase">
                {text}
              </span>
              <div className="h-4 w-[1px] bg-white/20"></div>
              <span className="text-blue-500 font-black text-[10px]">WRF</span>
              <div className="h-4 w-[1px] bg-white/20"></div>
            </div>
          ))}
        </div>
