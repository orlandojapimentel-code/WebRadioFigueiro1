...
const RequestCenter: React.FC = () => {
  ...
  return (
    <div className="bg-black/60 backdrop-blur-3xl rounded-[2rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col transition-all h-auto min-h-[550px]">
      <div className="bg-gradient-to-br from-blue-600 via-indigo-700 to-indigo-900 p-6 text-white relative overflow-hidden">
      ...
      <div className="p-6 space-y-8 flex-grow">
        ...
        <div className="h-[1px] bg-white/5 relative">
          <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 px-4 bg-black text-[9px] font-black text-slate-400 uppercase tracking-widest">{t.request.orSite}</span>
        </div>

        <form className="cc_request_form space-y-4" data-username="orlando">
           <div data-type="result" className="text-[10px] font-bold text-center py-2 text-blue-400 empty:hidden"></div>
           
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-1">
               <label className="text-[8px] font-black text-slate-400 uppercase ml-2">{t.request.artist}</label>
               <input type="text" name="request[artist]" className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 text-xs text-white" maxLength={127} />
             </div>
             <div className="space-y-1">
               <label className="text-[8px] font-black text-slate-400 uppercase ml-2">{t.request.song}</label>
               <input type="text" name="request[title]" className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 text-xs text-white" maxLength={127} />
             </div>
           </div>

           <div className="space-y-1">
             <label className="text-[8px] font-black text-slate-400 uppercase ml-2">{t.request.dedication}</label>
             <input type="text" name="request[dedication]" className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 text-xs text-white" maxLength={127} />
           </div>

           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-1">
               <label className="text-[8px] font-black text-slate-400 uppercase ml-2">{t.request.name}</label>
               <input type="text" name="request[sender]" className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 text-xs text-white" maxLength={127} />
             </div>
             <div className="space-y-1">
               <label className="text-[8px] font-black text-slate-400 uppercase ml-2">{t.request.email}</label>
               <input type="text" name="request[email]" className="w-full bg-black/40 border border-white/5 rounded-xl px-5 py-3 text-xs text-white" maxLength={127} />
             </div>
           </div>

           <button type="button" data-type="submit" className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.3em] transition-all shadow-xl active:scale-[0.98]">
             {t.request.submit}
           </button>
        </form>
      </div>

      <div className="p-5 bg-black/20 text-center border-t border-white/5">
