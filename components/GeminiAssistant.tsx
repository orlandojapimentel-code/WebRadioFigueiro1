...
const RequestCenter: React.FC = () => {
  const handleWhatsApp = () => {
    ...
    const text = encodeURIComponent("Olá Web Rádio Figueiró! Gostaria de fazer um pedido de música ou enviar uma dedicatória.");
    ...
    <div className="bg-white dark:bg-gray-800/40 rounded-[2.5rem] border border-gray-200 dark:border-blue-500/20 shadow-xl overflow-hidden flex flex-col transition-all">
      ...
      <div className="p-5 bg-gradient-to-r from-blue-600 to-blue-700 text-white">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className="font-black text-sm uppercase tracking-widest">Peça a Sua Música</h4>
            <p className="text-[9px] opacity-70 font-bold uppercase tracking-tighter">Ouvintes em primeiro lugar</p>
          </div>
        </div>
      </div>

      <div className="p-6 space-y-6">
        {/* Botão WhatsApp - Mais Rápido */}
        <button 
        ...
        {/* Formulário Oficial Centova Cast */}
        <form className="cc_request_form space-y-4" data-username="orlando">
          <div data-type="result" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 mb-2 empty:hidden bg-blue-500/10 p-4 rounded-xl border border-blue-500/20 text-center animate-pulse"></div>
          
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Artista</label>
              <input type="text" name="request[artist]" className="w-full bg-slate-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-900 dark:text-white" placeholder="Quem canta?" />
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Música</label>
              <input type="text" name="request[title]" className="w-full bg-slate-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-900 dark:text-white" placeholder="Título da música" />
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Dedicado para</label>
              <input type="text" name="request[dedication]" className="w-full bg-slate-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-900 dark:text-white" placeholder="Alguém especial?" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Seu Nome</label>
                <input type="text" name="request[sender]" className="w-full bg-slate-50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none text-slate-900 dark:text-white" placeholder="Seu nome" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-1">Seu E-Mail</label>
          ...
          <button 
            type="button" 
            data-type="submit"
            className="w-full bg-blue-600 dark:bg-white text-white dark:text-slate-900 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all hover:bg-blue-700 active:scale-95 shadow-lg shadow-blue-600/20 mt-2"
          >
            Submeter Pedido
          </button>
        </form>
