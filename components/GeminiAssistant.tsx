...
const GeminiAssistant: React.FC = () => {
  const defaultMessage: ChatMessage = { 
    role: 'model', 
    text: 'Sintonizados! Sou o Figueiró AI. 🎙️ Queres pedir uma música ou fazer uma dedicatória?' 
  };
  ...
  const handleSendMessage = async (text: string) => {
    ...
    // Guardamos o histórico antes da mensagem de erro para o próximo envio
    ...
    try {
      const responseText = await getRadioAssistantResponse(historyForAPI, text);
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="flex items-center space-x-1">
              ...
              <span className="text-[8px] text-blue-200 font-bold uppercase">No Ar • Estúdio</span>
            </div>
          </div>
        </div>
        <button onClick={resetChat} className="text-white/40 hover:text-white transition-colors">
      ...
      <div className="p-3 bg-gray-900/50 border-t border-white/5">
        <div className="flex overflow-x-auto space-x-2 pb-3 scrollbar-hide">
          {quickActions.map((q, i) => (
            <button key={i} onClick={() => handleSendMessage(q.prompt)} className="flex-shrink-0 bg-white/5 hover:bg-blue-600/20 border border-white/10 rounded-full px-3 py-1 text-[9px] font-bold text-blue-300 transition-all">
              {q.label}
            </button>
          ))}
        </div>
        <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(input); }} className="flex space-x-2">
          <input 
            type="text" 
            placeholder="Fala com o estúdio..." 
            value={input}
            onChange={(e) => setInput(e.target.value)}
            disabled={isTyping}
            className="flex-grow bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs text-white focus:outline-none focus:border-blue-500/50"
          />
          <button type="submit" disabled={isTyping || !input.trim()} className="bg-blue-600 text-white p-2.5 rounded-xl hover:bg-blue-500 transition-all disabled:opacity-20">
